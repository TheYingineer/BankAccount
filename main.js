...
class BankAccount {
    constructor(accountNumber, owner){
        this.accountNumber = accountNumber //string
        this.owner = owner // string; 
        this.transaction = [] // empty array
    }
...
// balance() - This method does not take any input, and returns the current balance on the account. The balance is computed by summing up the amounts in the transactions array. 
    ...
    balance(){ //no input
        ...
        for (let i = 0; i<this.transaction.length; i++){
            sum = sum + this.transaction[i]        // The balance is computed by summing up the amounts in the transactions array
        }
        return sum; 
        // 
    }
...
// NOTE: You should not be able to deposit a negative amount
        ...
        deposit(amt) {
            // this.amt = amt;
            if(amt > 0) {
            new Transaction(amt,"deposit") // push the transaction amount into transaction array
            }
            else {
...
// charge(payee, amt) - This method takes in the payee and amount, creates a new transaction with the payee and amount, and adds the transaction to the transaction array.
        ...
        charge(payee,amt){
            if(this.balance()>amt){ // NOTE: You should not be able to charge an amount that would make your balance dip below 0
            new Transaction(-amt,payee) // amount - The amount of the transaction. Positive amounts are money going into the account (deposit, refund). Negative amounts are money coming out of the account (a charge or debit).            
            }
        }
}
